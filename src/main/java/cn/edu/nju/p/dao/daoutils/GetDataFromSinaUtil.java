...
//                    System.out.println(stock);
                    ...
                    double lastClose = Double.valueOf(datas[2]);//昨日收盘价
                    ...
                    stockPO.setLastClose(lastClose);
                    stockPO.setCode(code);
                    stockPO.setName(adjustName(name));
                    stockPO.setCurrentPrice(currentPrice);
                    stockPO.setDate(date);
                    stockPO.setOpen(open);
                    stockPO.setHigh(high);
                    stockPO.setLow(low);
                    stockPO.setAdj_close(currentPrice);
                    stockPO.setClose(currentPrice);
                    ...
                    stockPO.setIsOpen(getIsOpen(volume));
...
//            }
    ...
    public boolean getIsOpen(long volume) {
        return volume == 0 ? false : true;
    }

    public List<StockPO> getPOList(String[] codeList) {
        List<StockPO> poList = new ArrayList<>();
        for (String data : codeList) {
            String code = data.substring(0, 6);
            String market = "sz";
            poList.add(getStockCurrentData(code, market));
            //当前线程等待0.5秒
            try {
                Thread.sleep(5L * 100L);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }
        System.out.println("POLIST添加完毕，共有" + poList.size() + "支股票");
        return poList;
    }
